...
function [Pair_Idx] = Adv_Opt_Hodge(Obs, Target, C_Matrix, No_item, R_alpha, R_beta, Eps_Greedy)
    ...
    % Theta = RobustEstimation(Target, Obs./sum(Obs), C_Matrix, 0, R_beta, 0);
    % Theta = HodgeRank(C_Matrix, ones(length(Obs), 1), Obs');
    % Theta = Spectral_Ranking(Obs, length(Target));
    ...
    Emp_Simplex = Mirror_Descent_Hodge(Theta, Target, C_Matrix, No_item, No_Iter, R_beta, 0.25);
