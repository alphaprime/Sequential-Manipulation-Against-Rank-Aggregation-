...
Offline_Hodge_Theta      = HodgeRank(A, y, W);
Offline_Hodge_Ranking    = score_to_ranking(Offline_Hodge_Theta);
Offline_Hodge_Score      = eval_ranking(true_r', Offline_Hodge_Ranking, top_K);
...
Rnd_Hodge_Theta = cell(T, 1);
Smp_Hodge_Theta = cell(T, 1);
Gdy_Hodge_Theta = cell(T, 1);
Adv_Hodge_Theta = cell(T, 1);

Rnd_Hodge_Ranking = cell(T, 1); 
Smp_Hodge_Ranking = cell(T, 1);
Gdy_Hodge_Ranking = cell(T, 1);
Adv_Hodge_Ranking = cell(T, 1);

Rnd_Hodge_Score = cell(T, 1);
Smp_Hodge_Score = cell(T, 1);
Gdy_Hodge_Score = cell(T, 1);
Adv_Hodge_Score = cell(T, 1);

Rnd_Hodge_Theta_Per_Turn = cell(T, 1);
Smp_Hodge_Theta_Per_Turn = cell(T, 1);
Gdy_Hodge_Theta_Per_Turn = cell(T, 1);
Adv_Hodge_Theta_Per_Turn = cell(T, 1);

Rnd_Hodge_Ranking_Per_Turn = cell(T, 1); 
Smp_Hodge_Ranking_Per_Turn = cell(T, 1);
Gdy_Hodge_Ranking_Per_Turn = cell(T, 1);
Adv_Hodge_Ranking_Per_Turn = cell(T, 1);

Rnd_Hodge_Score_Per_Turn = cell(T, 1);
Smp_Hodge_Score_Per_Turn = cell(T, 1);
Gdy_Hodge_Score_Per_Turn = cell(T, 1);
Adv_Hodge_Score_Per_Turn = cell(T, 1);

Rnd_Hodge_Seq_Per_Turn = cell(T, 1);
Smp_Hodge_Seq_Per_Turn = cell(T, 1);
Gdy_Hodge_Seq_Per_Turn = cell(T, 1);
Adv_Hodge_Seq_Per_Turn = cell(T, 1);

% Generate the goal 
Tmp_Ranking = [8;9;10;7;5;6;4;3;2;1];
for j = 1:length(Tmp_Ranking)
    % Re-assign the latent preference score
    Score_Target(Tmp_Ranking(j)) = Offline_Hodge_Theta(Offline_Hodge_Ranking(j));
end
...
for t = 1:T
    ...
    for s = 1:Turn
        ...
        while stop_time < stop_thred && action_flag == 1
            Adv_ID = Adv_Opt_Hodge(Adv_W, Score_Target, A, n, R_alpha, R_beta, Eps_Greedy);
    ...
    Rnd_Hodge_Theta_Per_Turn{t} = Rnd_Theta_Turn;
    Smp_Hodge_Theta_Per_Turn{t} = Smp_Theta_Turn;
    Gdy_Hodge_Theta_Per_Turn{t} = Gdy_Theta_Turn;
    Adv_Hodge_Theta_Per_Turn{t} = Adv_Theta_Turn;  

    Rnd_Hodge_Ranking_Per_Turn{t} = Rnd_Ranking_Turn;  
    Smp_Hodge_Ranking_Per_Turn{t} = Smp_Ranking_Turn; 
    Gdy_Hodge_Ranking_Per_Turn{t} = Gdy_Ranking_Turn; 
    Adv_Hodge_Ranking_Per_Turn{t} = Adv_Ranking_Turn;     

    Rnd_Hodge_Score_Per_Turn{t} = Rnd_Score_Turn;
    Smp_Hodge_Score_Per_Turn{t} = Smp_Score_Turn;
    Gdy_Hodge_Score_Per_Turn{t} = Gdy_Score_Turn;
    Adv_Hodge_Score_Per_Turn{t} = Adv_Score_Turn;

    Rnd_Hodge_Seq_Per_Turn{t} = W_Rnd_per_Turn;
    Smp_Hodge_Seq_Per_Turn{t} = W_Smp_per_Turn;
    Gdy_Hodge_Seq_Per_Turn{t} = W_Gdy_per_Turn;
    Adv_Hodge_Seq_Per_Turn{t} = W_Adv_per_Turn;
    ...
    % Evaluation
    Rnd_Theta   = HodgeRank(A, y, Rnd_W);
    Rnd_Ranking = score_to_ranking(Rnd_Theta);
    Rnd_Score   = eval_ranking(Rank_Target, Rnd_Ranking, top_K);
    Rnd_Hodge_Theta{t}   =  Rnd_Theta;   
    Rnd_Hodge_Ranking{t} =  Rnd_Ranking; 
    Rnd_Hodge_Score{t}   =  Rnd_Score; 

    Smp_Theta   = HodgeRank(A, y, Smp_W);
    Smp_Ranking = score_to_ranking(Smp_Theta);
    Smp_Score   = eval_ranking(Rank_Target, Smp_Ranking, top_K);
    Smp_Hodge_Theta{t}   =  Smp_Theta;
    Smp_Hodge_Ranking{t} =  Smp_Ranking;
    Smp_Hodge_Score{t}   =  Smp_Score;

    Gdy_Theta   = HodgeRank(A, y, Gdy_W);
    Gdy_Ranking = score_to_ranking(Gdy_Theta);
    Gdy_Score   = eval_ranking(Rank_Target, Gdy_Ranking, top_K);
    Gdy_Hodge_Theta{t}   =  Gdy_Theta;
    Gdy_Hodge_Ranking{t} =  Gdy_Ranking;
    Gdy_Hodge_Score{t}   =  Gdy_Score;

    Adv_Theta   = HodgeRank(A, y, Adv_W);
    Adv_Ranking = score_to_ranking(Adv_Theta);
    Adv_Score   = eval_ranking(Rank_Target, Adv_Ranking, top_K);
    Adv_Hodge_Theta{t}   =  Adv_Theta;
    Adv_Hodge_Ranking{t} =  Adv_Ranking;
    Adv_Hodge_Score{t}   =  Adv_Score;
    disp('Finish Attack.')
end

save('simulation_hodge_230212_v4.mat');